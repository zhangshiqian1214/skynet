...
function _decode_type_meta:__index(key)
	...
	local func = assert(_reader[t],string.format("%s, %s",self._CType, key))(msg, key)
	self[key] = func
	return func
end
...
function _encode_type_meta:__index(key)
	local t, msg = c._env_type(P, self._CType, key)
	local func = assert(_writer[t],string.format("%s, %s",self._CType, key))(msg, key)
	self[key] = func
	return func
end
...
function M.encode_ex(message, t, offset, func, ...)
	local encoder = c._wmessage_new(P, message)
	assert(encoder ,  message)
	c._offset(encoder, offset)
	encode_message(encoder, message, t)
	if func then
		local buffer, len = c._wmessage_buffer(encoder)
		local ret = func(buffer, len, ...)
...
function M.encode_int8(buffer, offset, value)
	return c._encode_int8(buffer, offset, value)
end

function M.encode_int16(buffer, offset, value)
	return c._encode_int16(buffer, offset, value)
end

function M.encode_int32(buffer, offset, value)
	return c._encode_int32(buffer, offset, value)
end

function M.encode_int64(buffer, offset, value)
	return c._encode_int64(buffer, offset, value)
end
...
local function check_table_fields(t, fields)
	for key, value in pairs(fields) do
		if t[key] == nil then
			return false
		end
		if type(value) == "table" then
			if not check_table_fields(t[key], value) then
				return false
			end
		end
	end
...
local message_field_cache = {}
function M.decode_ex(typename, buffer, ...)
	local ret = {}
	local ok = c._decode_ex(P, decode_message_cb , ret , typename, buffer, ...)
	if ok then
		ret = setmetatable(ret , default_table(typename))
		-- local fields = message_field_cache[typename]
		-- if fields == nil then
			-- fields = c._get_message_fields(P, typename)
			-- message_field_cache[typename] = fields
		-- end
		-- if not check_table_fields(ret, fields) then
			-- return false, "some required field is missing"
		-- end
		return ret
	else
		return false , c._last_error(P)
...
function M.decode_int8(buffer, ...)
	return c._decode_int8(buffer, ...)
end

function M.decode_int16(buffer, ...)
	return c._decode_int16(buffer, ...)
end

function M.decode_int32(buffer, ...)
	return c._decode_int32(buffer, ...)
end

function M.decode_int64(buffer, ...)
	return c._decode_int64(buffer, ...)
end
