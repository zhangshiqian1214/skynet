...
static int l_md5(lua_State *L) {
  ...
  unsigned char hex_buf[MD5_DIGEST_SIZE*2+1] = {0};

  const char *in = luaL_checkstring(L, 1);
  luaL_Buffer buf;
  luaL_buffinit(L, &buf);

  MD5Calc((unsigned char *)in, strlen(in), out);
...
static int l_sha1(lua_State *L) {
  ...
  unsigned char hex_buf[SHA1_DIGEST_SIZE*2+1] = {0};

  const char *in = luaL_checkstring(L, 1);
  luaL_Buffer buf;
  luaL_buffinit(L, &buf);

  SHA1Calc((unsigned char *)in, strlen(in), (unsigned char *)out);
...
static int l_sha224(lua_State *L) {
  ...
  unsigned char hex_buf[SHA224_DIGEST_SIZE*2+1] = {0};

  const char *in = luaL_checkstring(L, 1);
  luaL_Buffer buf;
  luaL_buffinit(L, &buf);

  SHA224_Simple(in, strlen(in), (unsigned char *)out);
...
static int l_sha256(lua_State *L) {
  ...
  unsigned char hex_buf[SHA256_DIGEST_SIZE*2+1] = {0};

  const char *in = luaL_checkstring(L, 1);
  luaL_Buffer buf;
  luaL_buffinit(L, &buf);

  SHA256_Simple(in, strlen(in), (unsigned char *)out);
...
static int l_sha384(lua_State *L) {
  ...
  unsigned char hex_buf[SHA384_DIGEST_SIZE*2+1] = {0};

  const char *in = luaL_checkstring(L, 1);
  luaL_Buffer buf;
  luaL_buffinit(L, &buf);

  SHA384_Simple(in, strlen(in), (unsigned char *)out);
...
static int l_sha512(lua_State *L) {
  ...
  unsigned char hex_buf[SHA512_DIGEST_SIZE*2+1] = {0};

  const char *in = luaL_checkstring(L, 1);
  luaL_Buffer buf;
  luaL_buffinit(L, &buf);

  SHA512_Simple(in, strlen(in), (unsigned char *)out);
...
static int l_hmac_md5(lua_State *L) {
  ...
  hmac_md5((unsigned char *)key, strlen(key), (unsigned char *)in, strlen(in), out);
  tohex(out, hex_buf, MD5_DIGEST_SIZE);
...
static int l_hmac_sha1(lua_State *L) {
  ...
  hmac_sha1((unsigned char *)key, strlen(key), (unsigned char *)in, strlen(in), out);
  tohex(out, hex_buf, SHA1_DIGEST_SIZE);
...
static int l_hmac_sha224(lua_State *L) {
  ...
  hmac_sha224((unsigned char *)key, strlen(key), (unsigned char *)in, strlen(in), out);
  tohex(out, hex_buf, SHA224_DIGEST_SIZE);
...
static int l_hmac_sha256(lua_State *L) {
  ...
  hmac_sha256((unsigned char *)key, strlen(key), (unsigned char *)in, strlen(in), out);
  tohex(out, hex_buf, SHA256_DIGEST_SIZE);
...
static int l_hmac_sha384(lua_State *L) {
  ...
  hmac_sha384((unsigned char *)key, strlen(key), (unsigned char *)in, strlen(in), out);
  tohex(out, hex_buf, SHA384_DIGEST_SIZE);
...
static int l_hmac_sha512(lua_State *L) {
  ...
  hmac_sha512((unsigned char *)key, strlen(key), (unsigned char *)in, strlen(in), out);
  tohex(out, hex_buf, SHA512_DIGEST_SIZE);
...
static int l_crc16(lua_State *L) {
  ...
  uint16_t out = crc16((unsigned char *)in, strlen(in));
...
static int l_crc32(lua_State *L) {
  ...
  uint32_t out = crc32((unsigned char *)in, strlen(in));
  lua_pushnumber(L, out);
  return 1;
}

// Creates a new longnumber and pushes it onto the statck
static uint64_t * lualongnumber_pushlong(lua_State *L, uint64_t *val) {
...
static int l_crc64(lua_State *L) {
  const char *in = luaL_checkstring(L, 1);
  uint64_t out = crc64((unsigned char *)in, strlen(in));
