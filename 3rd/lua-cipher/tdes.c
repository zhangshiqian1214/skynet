...
unsigned char triple_des_key[16]={
    0x44,0x2f,0x7c,0xda,
    0x9c,0xb0,0x98,0xe8,
    0x8f,0xc5,0xd4,0xfd,
    0x32,0x79,0x24,0x12
...
-----------------------------------------------------------*/
void expand(unsigned char in[4], unsigned char out[6])
{
    union hbyte ip1;
    union hbyte ipr;
    ip1.abyte=0;
...
Description:
-----------------------------------------------------------*/
static void compress(unsigned char in[6],unsigned char out[4])
{
    /* S Box */
...
Description:
-----------------------------------------------------------*/
void permutate(unsigned char in[4],unsigned char out[4])
{
    union hbyte ip1;
    union hbyte ipr;
    ip1.abyte=0;
...
-----------------------------------------------------------*/
void pc2(unsigned char keyc[4],unsigned char keyd[4],unsigned char subkey[6])
{
    union hbyte ip1;
    union hbyte ipr;
    ip1.abyte=0;
...
Description: Sub Procedure of Subkey
-----------------------------------------------------------*/
void rotatebits(unsigned char key[4],unsigned char skey[4],unsigned char bits)
{
    union hbyte c[4],d[4];
...
Description:
-----------------------------------------------------------*/
void Gsubkey(unsigned char key[8],unsigned char subkey[16][6])
{
    int i;
    unsigned char cup[4], dup[4];
    unsigned char ci[4],  di[4];
    unsigned char lsi[16];
    union hbyte ip1;
    union hbyte ipr;
    ip1.abyte=0;
    ipr.abyte=0;
...
Description:
-----------------------------------------------------------*/
void des(unsigned char m[8],unsigned char key[8])
{
    unsigned char  ip[8];
    unsigned char  lin[4];
    unsigned char  rin[4];
...
Description:
-----------------------------------------------------------*/
void undes(unsigned char m[8],unsigned char key[8])
{
    unsigned char  ip[8];
    unsigned char  lin[4];
    unsigned char  rin[4];
...
void SDes(unsigned char orientation, unsigned char PlainText[8], unsigned char Key[8], unsigned char Encipher[8])
{
    unsigned char m[8];
    unsigned char k[8];
...
{
    unsigned char t_plain[MAX_CI_LEN];
    unsigned char t_crypt[MAX_CI_LEN];
    int en_cnt;
    int pad_cnt=0;
    int i;
...
int cipher3(const unsigned char* key, unsigned char* plain_text, unsigned char* crypted_text,int length)
{
    ...
    int pad_cnt=0;
    int i;
    unsigned char usekey[32];

    for(i=0;i<32;i++)
        usekey[i]='\0';

    strncpy((char *)usekey, (char *)key, 16);
    if(length>MAX_CI_LEN) return -1;
