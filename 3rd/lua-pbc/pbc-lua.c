...
static int
_get_buffer(lua_State *L, struct pbc_slice* slice, int arg_start){
	int offset;
	...
	if (lua_type(L,arg_end) == LUA_TSTRING) {
		...
		slice->buffer = checkuserdata(L,arg_end);
...
static int
_decode_ex(lua_State *L) {
	struct pbc_env * env = (struct pbc_env *)checkuserdata(L,1);
	luaL_checktype(L, 2 , LUA_TFUNCTION);
	luaL_checktype(L, 3 , LUA_TTABLE);
	const char * type = luaL_checkstring(L,4);
	struct pbc_slice slice;
	int arg_start = _get_buffer(L, &slice, 5);
	if(arg_start == 0){
		lua_pushboolean(L,0);
		return 1;
	}
...
_offset(lua_State *L) {
	...
	pbc_wmessage_offset(m, offset);
	return 0;
}


static int
_encode_int8(lua_State *L) {
	struct pbc_slice slice;
...
_encode_int64(lua_State *L) {
    ...
    int64_t number = (int64_t)(luaL_checknumber(L,arg_start));
    uint32_t low = (uint32_t)number;
    uint32_t hi = (uint32_t)(number >> 32);
	unsigned char *buffer = (unsigned char *)slice.buffer;
	buffer[3] = (uint8_t)(low & 0xff);
	buffer[2] = (uint8_t)(low >> 8 & 0xff);
	buffer[1] = (uint8_t)(low >> 16 & 0xff);
	buffer[0] = (uint8_t)(low >> 24 & 0xff);
	buffer[7] = (uint8_t)(hi & 0xff);
	buffer[6] = (uint8_t)(hi >> 8 & 0xff);
	buffer[5] = (uint8_t)(hi >> 16 & 0xff);
	buffer[4] = (uint8_t)(hi >> 24 & 0xff);
...
_decode_int64(lua_State *L) {
	...
	unsigned char *buffer = (unsigned char *)slice.buffer;
	uint32_t low = ((uint32_t)buffer[0] << 24) + ((uint32_t)buffer[1] << 16) + ((uint32_t)buffer[2] << 8) + buffer[3];
    int64_t hi = ((uint32_t)buffer[4] << 24) + ((uint32_t)buffer[5] << 16) + ((uint32_t)buffer[6] << 8) + buffer[7];
...
static void
_fill_message_field(void* data, void *ud) {
	lua_State *L = (lua_State*)ud;
	struct _field* f = (struct _field*)data;
	if(f->label != LABEL_REQUIRED) {
		return;
	}
	if(f->type == PTYPE_MESSAGE) {
		struct _message *msg = f->type_name.m;
		lua_pushstring(L, f->name);
		lua_newtable(L);
		_pbcM_sp_foreach_ud(msg->name, _fill_message_field, L);
		lua_settable(L, -3);
	} else {
		lua_pushstring(L, f->name);
		lua_pushboolean(L, 1);
...
static int
_get_message_fields(lua_State *L) {
	struct pbc_env * env = (struct pbc_env *)checkuserdata(L,1);
	const char* type_name = luaL_checkstring(L, 2);
...
luaopen_protobuf_c(lua_State *L) {
	luaL_Reg reg[] = {
		...
		{"_offset", _offset },
		{"_encode_int8", _encode_int8 },
		{"_encode_int16", _encode_int16 },
		{"_encode_int32", _encode_int32 },
        {"_encode_int64", _encode_int64 },
		{"_decode_ex", _decode_ex },
		{"_decode_int8", _decode_int8 },
		{"_decode_int16", _decode_int16 },
		{"_decode_int32", _decode_int32 },
        {"_decode_int64", _decode_int64 },
		{"_get_message_fields", _get_message_fields },
        {"_memory", _memory },
		{NULL,NULL},
	};
